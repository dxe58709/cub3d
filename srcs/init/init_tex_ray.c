...
/*   Updated: 2024/12/15 22:40:35 by nsakanou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

static void	initialize_tex_ray(t_tex_ray *tex_ray)
{
	tex_ray->x = 0;
	tex_ray->y = 0;
	tex_ray->next = 0.0;
...
void	init_tex_ray(t_game *game, t_ray *ray, t_tex_ray *tex_ray)
{
	...
	if ((ray->side == 0 && ray->x_vec_dir < 0) || (ray->side == 1
			&& ray->y_vec_dir > 0))
		tex_ray->x = tex_size - tex_ray->x - 1;
	tex_ray->next = 1.0 * tex_size / ray->wall_height;
